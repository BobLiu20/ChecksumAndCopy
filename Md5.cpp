...
//转义'\'
CString strChange(CString before)
{
	CString after;
	after.Empty();
	int ndx;
	while( (ndx = before.Find('\\')) > 0)
	{
		after += before.Left(ndx+1);
		after += '\\';
		before = before.Mid(ndx+1);	
	}
	after += before;
    return after;	
}
...
CString ValueMD5(CString file)
{
	...
	TCHAR chtmp[1024];
	file = strChange(file);
	FILE *fp = _tfopen(file.LockBuffer(), _T("rb"));
	if(fp)
	{
		val = md5File(fp);	
		_stprintf(chtmp, _T("%08x%08x%08x%08x\n"), conv(val.a), conv(val.b), conv(val.c), conv(val.d));
		fclose(fp);
	}
	result.Format(_T("%s"), chtmp);
	return result;
